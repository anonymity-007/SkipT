...
class ResultParser(object):
    ...
    def read_accs_base_to_new(self):
        ...
        df.loc[len(df.index)] = ['average'] + df.drop(columns=['dataset']).mean().tolist()
        for seed in seeds:
            df.loc[len(df.index) - 1, f'H_seed{seed}'] = 2 / (1 / df.loc[len(df.index) - 1, f'base_acc_seed{seed}'] 
                                                            + 1 / df.loc[len(df.index) - 1, f'new_acc_seed{seed}'])
