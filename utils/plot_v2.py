...
class Ploter(object):
    def __init__(self, dir_, prefix, variables, save_path, mode):
        self.dir_ = dir_
        self.variables = variables
        self.save_path = save_path
        self.mode = mode
        suffix = '_'.join([rf'{var}\=.*' for var in variables]) + '\.csv'
        pattern = '-'.join([prefix, suffix])
        self.pattern = pattern.replace('+', r'\+').replace('-', r'\-')
    ...
    def parse_filename(self, filename):
        ...
        for variable in self.variables:
            pattern = rf'{variable}\=[^_]*'
            value = re.findall(pattern, var_and_values)[0].split('=')[1]
...
def main(args):
    ploter = Ploter(args.dir, args.prefix, args.variables, args.save_path, args.mode)
    ploter.plot()
